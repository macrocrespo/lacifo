...
class Compra extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('usuario_id, proveedor_id', 'required'),
			array('id, usuario_id, proveedor_id, estado', 'numerical', 'integerOnly'=>true),
			array('total', 'length', 'max'=>10),
			array('observacion', 'length', 'max'=>100),
			array('fecha', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, fecha, total, observacion, usuario_id, proveedor_id, estado', 'safe', 'on'=>'search'),
		);
	}
